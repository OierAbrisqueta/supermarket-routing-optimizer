...
-- Store Table
CREATE TABLE IF NOT EXISTS Store (id INTEGER PRIMARY KEY, name TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS Area (id INTEGER PRIMARY KEY, store_id INTEGER,
                                        x REAL, y REAL, FOREIGN KEY(store_id) REFERENCES Store(id));

-- Edge table
CREATE TABLE IF NOT EXISTS Edge (id INTEGER PRIMARY KEY, store_id INTEGER,
                                        node1_id INTEGER, node2_id INTEGER, weight REAL,
                                        FOREIGN KEY(store_id) REFERENCES Store(id), FOREIGN KEY(node1_id) REFERENCES Area(id),
...
CREATE TABLE IF NOT EXISTS Inventory (store_id INTEGER, product_id INTEGER, node_id INTEGER,
                                      PRIMARY KEY (store_id, product_id), FOREIGN KEY(store_id) REFERENCES Store(id),
